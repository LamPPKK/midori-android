...
internal class NativeAdBlockMatcher private constructor(
    ...
    /**
     * The engine intentionally lives for the application process. `shouldInterceptRequest` can
     * run concurrently, so freeing/replacing it during shutdown would race WebView worker threads.
     */
...
internal object AdBlockRequestPolicy {
    ...
    fun bypassBeforeHost(isForMainFrame: Boolean): Boolean = isForMainFrame

    fun isSupportedWebScheme(value: String?): Boolean =
        value.equals("http", ignoreCase = true) || value.equals("https", ignoreCase = true)
    ...
    fun create(
        ...
        val safeMethod = method
            ...
            ?.lowercase(Locale.ROOT)
            ?: return null
        // The native boundary requires a non-empty source. Treating an unknown/invalid source as
        // the target keeps classification conservatively first-party instead of over-blocking.
    ...
    private fun inferResourceType(headers: Map<String, String>, path: String): AdBlockResourceType {
        val destination = header(headers, "Sec-Fetch-Dest")?.lowercase(Locale.ROOT)
        when (destination) {
            ...
            "script", "serviceworker", "sharedworker", "worker" -> return AdBlockResourceType.SCRIPT
...
/**
 * Process-wide Android content-blocking host.
 *
 * Production artifacts package the reviewed Rust core and call its C ABI through JNA. The bundled
 * matcher remains a last-known-good safety net and intentionally supports only bounded
 * `||domain^`, `@@||domain^`, and their `$third-party` variants.
 */
internal class AdBlockCoordinator private constructor(context: Context) {
    ...
    /**
     * Installs a process-lifetime matcher that is safe for concurrent calls. Replacements are not
     * closed here: native owners must keep an old engine alive until process shutdown so a worker
     * that already captured it can finish without racing a free.
     */
    ...
    @Suppress("NewApi")
    fun installProfileServiceWorkerClient(webView: WebView) {
    ...
    fun shouldIntercept(request: WebResourceRequest, sourceUrl: String?): WebResourceResponse? {
        if (AdBlockRequestPolicy.bypassBeforeHost(request.isForMainFrame)) return null
        val targetUrl = request.url.toString()
        val blocked = try {
            if (AdBlockRequestPolicy.exceedsNativeUrlLimit(targetUrl)) {
                if (!AdBlockRequestPolicy.isSupportedWebScheme(request.url.scheme)) return null
...
internal class BundledAbpDomainMatcher private constructor(
    ...
    override fun shouldBlock(request: AdBlockMatchRequest): Boolean {
        return shouldBlockHosts(request.targetHost, request.sourceHost)
    }
    ...
    private fun isThirdParty(targetHost: String, sourceHost: String): Boolean {
        // The reviewed bootstrap list contains only .com/.net domains. The JNI engine will replace
        // this bounded approximation with adblock-rust's full party/domain handling.
        return approximateSite(targetHost) != approximateSite(sourceHost)
    }
    ...
    companion object {
        ...
        internal fun fromText(text: String): BundledAbpDomainMatcher =
            parse(text) ?: empty()

        private fun parse(text: String): BundledAbpDomainMatcher? {
            if (text.length > MAX_RULE_BYTES || text.toByteArray(StandardCharsets.UTF_8).size > MAX_RULE_BYTES) {
                return null
            }
